        ...
        <div>
            ...
            <div class="grid w-full grid-flow-row grid-cols-10 gap-2 mx-2">
                @foreach ($categories as $category)
                    <button class="flex flex-col justify-between p-2 bg-gray-800 border rounded w-fit hover:bg-gray-600" wire:click="$dispatch('openModal', {component: 'modal.manage.edit-category', arguments: ['{{ $category->id }}'] })">
                        <div class="w-full">
                            <h1>{{ $category->name }}</h1>
